...
pub fn happ_bundle() -> AppBundle {
    let bytes = include_bytes!("../../workdir/vines.happ");
    AppBundle::decode(bytes).expect("Failed to decode vines happ")
}
        ...
        .setup(|app| {
            ...
            let result: anyhow::Result<()> = tauri::async_runtime::block_on(async move {
                ...
                let mut window_builder = app.holochain()?
                    .main_window_builder(String::from("main"), false, Some(String::from("vines")), None)
                    .await?;

                #[cfg(desktop)]
                {
                    window_builder = window_builder.title(String::from("Vines"))
                }
...
fn holochain_dir() -> PathBuf {
    if tauri::is_dev() {
        ...
        {
            let tmp_dir =
                tempdir::TempDir::new("vines").expect("Could not create temporary directory");
